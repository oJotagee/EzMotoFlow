...
const motorcycleSchema = z.object({
  ...
  placa: z
    ...
    .regex(/^[A-Za-z]{3}-[0-9]{4}$/, "Placa deve estar no formato ABC-1234"),
  ano: z
    .string()
    .length(4, "Ano deve ter exatamente 4 dígitos")
  ...
  renavam: z
    ...
    .regex(/^\d{11}$/, "Renavam deve conter apenas números"),
...
export default function CreateMotorcyclePage() {
  ...
  const { mutate: save, isPending: sending } = useMutation({
    mutationFn: async (values: any) => {
      const valor_compra =
        typeof values.valor_compra === "number" ? values.valor_compra : 0;
      const valor_venda =
        typeof values.valor_venda === "number" ? values.valor_venda : 0;
      const valor_fipe =
        typeof values.valor_fipe === "number" ? values.valor_fipe : 0;
    ...
    <PermissionGuard
      ...
      fallback={
        <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
          ...
          <Link to="/motorcycles">
            <Button testID="back-to-motorcycles" type="secondary">

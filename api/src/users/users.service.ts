...
export class UsersService {
  ...
  async update(updateUserDto: UpdateUserDto, tokenPayload: PayloadDto): Promise<ResponseUpdateUserDto> {
    try {
      ...
      if (findUser.id !== tokenPayload.sub) throw new HttpException("You cannot update this user", HttpStatus.FORBIDDEN);
    ...
    try {
      ...
      if (!findUser) throw new HttpException("User not found", HttpStatus.NOT_FOUND);
      if (findUser.id !== tokenPayload.sub) throw new HttpException("You cannot delete this user", HttpStatus.FORBIDDEN);
  ...
  async uploadAvatarFile(file: Express.Multer.File, payloadToken: PayloadDto): Promise<ResponseUpdateAvatarDto> {
    try {
